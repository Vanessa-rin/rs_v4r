 ...
 * Optimized for the v4r Perception Pipelines
...
v4rProcessManager::v4rProcessManager(ros::NodeHandle n, const std::string savePath, std::string &name) :
        savePath(savePath),
        nh(n),
        image_transport(nh),
        ...
        name(name)
{
    ...
    vis_service = nh.advertiseService("vis_command", &v4rProcessManager::visControlCallback, this);
    image_pub = image_transport.advertise("result_image", 1, true);
    visualize = true;
}

v4rProcessManager::v4rProcessManager(ros::NodeHandle n, std::string &name) :
        nh(n),
        image_transport(nh),
        visualizer(savePath, true),
        name(name)
...
void v4rProcessManager::setup() {
    outInfo("A RoboSherlock process manager optimized for the v4r perception was created.");
...
void v4rProcessManager::init(std::string &pipeline) {
...
void v4rProcessManager::run(std::map<std::string, boost::any> args, std::vector<ObjectDetectionData>& detectionData) {
    outInfo("Running the v4r Process Manager");
...
bool v4rProcessManager::has_vertical_plane() {
...
void v4rProcessManager::getClusterFeatures(rs::ObjectHypothesis cluster, std::vector<ObjectDetectionData> &data) {
    ...
    if(!geometry.empty()) {
        ...
        if(!poses.empty()) {
            rs_v4r::conversion::from(poses[0].world.get(), poseStamped);
        } else {
        ...
        rs_v4r::conversion::makeObjectDetectionData(poseStamped, geometry[0],
                                                                rs_v4r::conversion::decode_shape(shapes), cluster.region(), objClass, confidence, c, odd);
        data.push_back(odd);
...
bool v4rProcessManager::visControlCallback(robosherlock_msgs::RSVisControl::Request &req,
